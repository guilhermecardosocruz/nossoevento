...
export const { handlers, auth } = NextAuth({
  ...
  callbacks: {
    async jwt({ token, user }) { if (user) (token as any).cpf = (user as any).cpf; return token; },
    async session({ session, token }) { (session as any).cpf = (token as any).cpf; return session; }
  }
});
